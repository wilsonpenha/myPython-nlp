...
class MyDisplay(object):
    def prepare(self, lda_model, dtm, vectorizer, mds):
        return prepare(lda_model, dtm, vectorizer, mds=mds)

    def prepare_glda(self, lda_model, dtm, vectorizer, mds):
        return prepare_glda(lda_model, dtm, vectorizer, mds=mds)
...
def glda_get_doc_topic_dists(lda_model, dtm):
    return _row_norm(lda_model.transform(dtm))
...
def glda_get_topic_term_dists(lda_model):
    return _row_norm(lda_model.topic_word_)

def glda_extract_data(lda_model, dtm, vectorizer):
    vocab = _get_vocab(vectorizer)
    doc_lengths = _get_doc_lengths(dtm)
    term_freqs = _get_term_freqs(dtm)
    topic_term_dists = glda_get_topic_term_dists(lda_model)
    ...
    doc_topic_dists = glda_get_doc_topic_dists(lda_model, dtm)
...
def prepare_glda(lda_model, dtm, vectorizer, **kwargs):
    """Create Prepared Data from sklearn's LatentDirichletAllocation and CountVectorizer.
    ...
    **kwargs: Keyword argument to be passed to pyLDAvis.prepare()
    ...
    See `pyLDAvis.prepare` for **kwargs.
    """
    opts = fp.merge(glda_extract_data(lda_model, dtm, vectorizer), kwargs)
    return pyLDAvis.prepare(**opts)
