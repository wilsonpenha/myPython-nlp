...
#         print()
    ...
    def save_lda_topic_model(self):
        joblib.dump(self.lda, 'glda_topic_model.lda') 
        
    def load_lda_topic_model(self):
        self.lda = joblib.load('glda_topic_model.lda')
    
    def save_term_vector_topic_model(self, vectorizer):
        joblib.dump(vectorizer, 'glda_vectorizer_topic_model.vtf') 
        
    def load_term_vector_topic_model(self):
        vectorizer = joblib.load('glda_vectorizer_topic_model.vtf')
        return vectorizer
    
    def save_fitted_term_vector(self, tf):
        joblib.dump(tf, 'glda_vectorizer_topic_model.ftf') 
        
    def load_fitted_term_vector(self):
        ftf = joblib.load('glda_vectorizer_topic_model.ftf')
        return ftf
...
#     tmt.build_lda(tmt.process_corpus(),20)
    tmt.build_lda(tmt.load_line_corpus("/data/corpus/full.txt"),35)
